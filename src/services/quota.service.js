...
class QuotaService {
  ...
  async upsertQuota(cookie_id, model_name, quotaInfo) {
    ...
    const reset_time = resetTime ? new Date(resetTime).toISOString() : null;

    try {
      await database.query(
        `INSERT INTO model_quotas (quota_id, cookie_id, model_name, reset_time, quota, status)
         VALUES (?, ?, ?, ?, ?, ?)
        ...
        [database.generateUUID(), cookie_id, model_name, reset_time, quota, status]
      );
      
      const result = await database.query(
        'SELECT * FROM model_quotas WHERE cookie_id = ? AND model_name = ?',
  ...
  async updateModelQuotaStatus(cookie_id, model_name, status) {
    try {
      await database.query(
        `UPDATE model_quotas SET status = ? WHERE cookie_id = ? AND model_name = ?`,
  ...
  async updateUserSharedQuotaMax(user_id, model_name) {
    try {
      ...
      const countResult = await database.query(
        `SELECT COUNT(*) as cookie_count FROM accounts
         WHERE user_id = ? AND is_shared = 1 AND status = 1`,
        [user_id]
      );
      const cookieCount = parseInt(countResult.rows[0]?.cookie_count) || 0;
      const newMax = 2.0 * cookieCount;
      
      // 更新或插入配额池记录
      await database.query(
        `INSERT INTO user_shared_quota_pool (pool_id, user_id, model_name, quota, max_quota)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT (user_id, model_name)
         DO UPDATE SET max_quota = excluded.max_quota, last_updated_at = datetime('now')`,
        [database.generateUUID(), user_id, model_name, newMax, newMax]
      );
      
      const result = await database.query(
        'SELECT * FROM user_shared_quota_pool WHERE user_id = ? AND model_name = ?',
        [user_id, model_name]
      );
  ...
  async addUserSharedQuota(user_id, model_name, account_quota) {
    try {
      ...
      if (existing.rows.length > 0) {
        // 更新
        const newQuota = Math.min(existing.rows[0].quota + quota_to_add, existing.rows[0].max_quota + max_quota_increment);
        const newMaxQuota = existing.rows[0].max_quota + max_quota_increment;
        await database.query(
          `UPDATE user_shared_quota_pool SET quota = ?, max_quota = ?, last_updated_at = datetime('now')
           WHERE user_id = ? AND model_name = ?`,
          [newQuota, newMaxQuota, user_id, model_name]
        );
      } else {
        // 插入
        const initialQuota = Math.min(quota_to_add, max_quota_increment);
        await database.query(
          `INSERT INTO user_shared_quota_pool (pool_id, user_id, model_name, quota, max_quota)
           VALUES (?, ?, ?, ?, ?)`,
          [database.generateUUID(), user_id, model_name, initialQuota, max_quota_increment]
        );
      }
  ...
  async removeUserSharedQuota(user_id, model_name, account_quota) {
    try {
      ...
      const newQuota = Math.max(existing.rows[0].quota - quota_to_remove, 0);
      const newMaxQuota = Math.max(existing.rows[0].max_quota - 2, 0);
      
      await database.query(
        `UPDATE user_shared_quota_pool
         SET quota = ?, max_quota = ?, last_updated_at = datetime('now')
         WHERE user_id = ? AND model_name = ?`,
        [newQuota, newMaxQuota, user_id, model_name]
      );
  ...
  async deductUserSharedQuota(user_id, model_name, amount) {
    try {
      ...
      await database.query(
        `UPDATE user_shared_quota_pool SET quota = quota - ?, last_updated_at = datetime('now')
         WHERE user_id = ? AND model_name = ?`,
        [amount, user_id, model_name]
      );
  ...
  async recoverUserSharedQuota(user_id, model_name) {
    try {
      ...
      const countResult = await database.query(
        `SELECT
           SUM(CASE WHEN paid_tier = 0 THEN 1 ELSE 0 END) as free_count,
           SUM(CASE WHEN paid_tier = 1 THEN 1 ELSE 0 END) as paid_count
         FROM accounts
         WHERE user_id = ?
           AND is_shared = 1
           AND status = 1`,
      ...
      const newQuota = Math.min(existing.rows[0].quota + recoveryAmount, existing.rows[0].max_quota);
      
      // 恢复配额（不超过上限）
      await database.query(
        `UPDATE user_shared_quota_pool
         SET quota = ?, last_recovered_at = datetime('now'), last_updated_at = datetime('now')
         WHERE user_id = ? AND model_name = ?`,
        [newQuota, user_id, model_name]
      );
  ...
  async recordQuotaConsumption(user_id, cookie_id, model_name, quota_before, quota_after, is_shared = 1) {
    try {
      ...
      const logId = database.generateUUID();
      await database.query(
        `INSERT INTO quota_consumption_log (log_id, user_id, cookie_id, model_name, quota_before, quota_after, quota_consumed, is_shared)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [logId, user_id, cookie_id, model_name, quota_before, quota_after, quota_consumed, is_shared]
      );
      
      const result = await database.query(
        'SELECT * FROM quota_consumption_log WHERE log_id = ?',
        [logId]
      );
