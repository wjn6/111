...
class KiroAccountService {
      ...
      if (existingAccount) {
        ...
        await database.query(
          `UPDATE kiro_accounts
           SET user_id = ?, account_name = ?, auth_method = ?, refresh_token = ?,
               access_token = ?, expires_at = ?, client_id = ?, client_secret = ?,
               profile_arn = ?, machineid = ?, is_shared = ?, email = ?,
               subscription = ?, current_usage = ?, reset_date = ?,
               usage_limit = ?, free_trial_status = ?, free_trial_usage = ?,
               free_trial_expiry = ?, free_trial_limit = ?, bonus_usage = ?,
               bonus_limit = ?, bonus_available = ?, bonus_details = ?,
          ...
          [user_id, account_name, auth_method, refresh_token, access_token, expires_at,
           ...
           subscription, current_usage, reset_date, usage_limit, free_trial_status ? 1 : 0,
        ...
        const result = await database.query('SELECT * FROM kiro_accounts WHERE userid = ?', [userid]);
        logger.info(`Kiro账号更新成功: account_id=${result.rows[0].account_id}, userid=${userid}`);
        return result.rows[0];
      } else {
        // 不存在则创建
        const account_id = database.generateUUID();
        await database.query(
          `INSERT INTO kiro_accounts
           (account_id, user_id, account_name, auth_method, refresh_token, access_token, expires_at,
        ...
        const result = await database.query('SELECT * FROM kiro_accounts WHERE account_id = ?', [account_id]);
  ...
  async getAvailableAccounts(user_id = null) {
    try {
      let query = 'SELECT * FROM kiro_accounts WHERE status = 1 AND need_refresh = 0';
      const params = [];
  ...
  async updateAccountToken(account_id, access_token, expires_at, profile_arn = null) {
    try {
      if (profile_arn) {
        await database.query(
          `UPDATE kiro_accounts 
           SET access_token = ?, expires_at = ?, profile_arn = ?, updated_at = datetime('now')
           WHERE account_id = ?`,
  ...
  async updateAccountUsage(account_id, usageData) {
    try {
      ...
      // 先获取当前记录
      const current = await database.query('SELECT * FROM kiro_accounts WHERE account_id = ?', [account_id]);
      ...
      await database.query(
        `UPDATE kiro_accounts
         SET email = ?, userid = ?, subscription = ?, current_usage = ?, reset_date = ?,
             usage_limit = ?, free_trial_status = ?, free_trial_usage = ?,
             free_trial_expiry = ?, free_trial_limit = ?, bonus_usage = ?,
             bonus_limit = ?, bonus_available = ?, bonus_details = ?,
             updated_at = datetime('now')
         WHERE account_id = ?`,
        [
          email ?? c.email, userid ?? c.userid, subscription ?? c.subscription,
          current_usage ?? c.current_usage, reset_date ?? c.reset_date,
          usage_limit ?? c.usage_limit, 
          (free_trial_status !== undefined ? (free_trial_status ? 1 : 0) : c.free_trial_status),
          free_trial_usage ?? c.free_trial_usage, free_trial_expiry ?? c.free_trial_expiry,
          free_trial_limit ?? c.free_trial_limit, bonus_usage ?? c.bonus_usage,
          bonus_limit ?? c.bonus_limit, bonus_available ?? c.bonus_available,
          bonus_details ? JSON.stringify(bonus_details) : c.bonus_details,
          account_id
        ]
      );
